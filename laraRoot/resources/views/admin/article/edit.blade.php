...
<div class="container">
    <div class="row">
        ...
        @endif
        <h3 class="margin-fix">编辑文章<a href="{{route('admin.article.create')}}" class="pull-right">新增文章</a></h3>
    </div>
    <!-- markdown -->
    <div class="row">
        <form class="edit-form" action="{{route('admin.article.update',$article->id)}}" method="POST">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="cate" id="cateOption" title="分类"
                    ...
                    @foreach($category as $v)
                        <option value="{{$v->id}}"
                                @if($article->category_id==$v->id) selected @endif>{{$v->title}}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="tag" cols="30" rows="2" ondrop="false" title="tags">{{$article->tag}}</textarea>
